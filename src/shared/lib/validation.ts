...
export const eventCreateSchema=z.object({
  ...
  // FIX: Updated for Zod v4 compatibility
  time: z.preprocess((val) => {
    if (!val || val === 'null' || val === 'undefined') return '';
    return String(val).trim();
  }, z.string().refine((val) => {
    if (val === '') return true; // Empty is allowed (optional)
    return /^\d{2}:\d{2}$/.test(val); // If present, must be HH:MM
  }, { message: 'Time must be in HH:MM format' }).default('')),
  
  location:cleanText(120),
  locationUrl:z.string().trim().max(2000).refine(value=>!value||value.startsWith('https://'),'Location URL must use HTTPS').optional().default(''),
  venue:cleanText(160),
  category:cleanText(50),
  ...
  passes:z.array(z.object({
    ...
    benefits:z.string().trim().max(500).optional().default(''),
    available:z.number().int().min(1).max(1_000_000),
    transferAllowed:z.boolean().optional().default(true)
  })).min(1).max(20),
});
...
export const scannerSchema=z.object({
  ...
  gate:z.string().trim().min(1).max(80).optional().default('Main Gate')
});
