...
export async function fulfillPaidOrder(providerOrderId:string,paymentId:string) {
  return db.transaction(async tx=>{
    ...
    const [claimed]=await tx.update(orders).set({paymentStatus:'SUCCESS',orderStatus:'PAID',transactionId:paymentId})
      .where(and(eq(orders.id,order.id),eq(orders.orderStatus,'CREATED'))).returning();
    if(!claimed) {
    ...
    for(const item of items){
      const [reserved]=await tx.update(passTypes).set({
        available:sql`${passTypes.available} - ${item.quantity}`,
        sold:sql`${passTypes.sold} + ${item.quantity}`,
      }).where(and(eq(passTypes.id,item.passTypeId),gte(passTypes.available,item.quantity))).returning();
