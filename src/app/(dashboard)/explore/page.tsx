...
export default function ExplorePage(){
  ...
  const load=useCallback(async()=>{setLoading(true);setError('');try{const response=await fetch('/api/data/content',{cache:'no-store'});if(!response.ok)throw new Error();const data=await response.json();setEvents((data.events||[]).sort((a:Event,b:Event)=>+new Date(a.date)-+new Date(b.date)));setTabs([{id:'all',label:'Everything'},...data.categories.map((item:{name:string})=>({id:item.name,label:item.name}))]);}catch{setError('Explore is unavailable right now.');}finally{setLoading(false);}},[]);
  useEffect(()=>{load();},[load]);
