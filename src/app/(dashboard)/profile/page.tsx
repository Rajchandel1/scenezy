...
import { CalendarDays, ChevronRight, Download, Monitor, Moon, QrCode, Sun, Ticket, UserRound, WalletCards } from 'lucide-react';
...
export default function ProfilePage(){
  ...
  return <div className="px-5 sm:px-6 pt-7 pb-8 space-y-8"><header><p className="eyebrow">Account</p><h1 className="display-serif text-white text-4xl mt-1">Your profile</h1></header>
    ...
    {!isInstalled&&<section className="editorial-card p-5"><div className="flex items-center gap-4"><span className="w-11 h-11 rounded-xl brand-button grid place-items-center"><Download size={18}/></span><div className="flex-1"><p className="text-sm font-semibold text-white">Install Scenezy</p><p className="muted text-xs mt-1">Faster access from your home screen.</p></div><button onClick={()=>canInstall?install():setShowInstallHelp(true)} className="rounded-full border border-[var(--line)] px-4 py-2 text-xs font-semibold">Install</button></div>{showInstallHelp&&<div className="mt-4 pt-4 border-t border-[var(--line)] text-xs muted leading-6"><p className="font-semibold text-[var(--ink)]">{requiresManualInstall?'Install on iPhone':'Install from your browser'}</p><p>{requiresManualInstall?'Open Scenezy in Safari, tap the Share icon, choose “Add to Home Screen”, then tap Add. iOS does not support an automatic install popup.':'Open the browser menu and choose “Install app” or “Add to Home Screen”.'}</p><button onClick={()=>setShowInstallHelp(false)} className="text-blue-400 mt-1">Got it</button></div>}</section>}
