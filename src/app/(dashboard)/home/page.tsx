...
function HomeSection({section}:{section:Section}){
  ...
  return <section><SectionHeading section={section}/><div className="divide-y divide-[var(--line)] border-y border-[var(--line)]">{items.map(event=><Link href={`/events/${event.id}`} key={event.id} className="py-3 flex items-center gap-4"><div className="display-serif text-center w-11"><span className="text-xl block">{new Date(event.date).getDate()}</span><span className="eyebrow">{new Date(event.date).toLocaleDateString('en-IN',{month:'short'})}</span></div><div className="min-w-0 flex-1"><h3 className="font-semibold text-sm truncate">{event.title}</h3><p className="muted text-[11px] mt-1 truncate">{event.time} · {event.venue}, {event.location}</p></div><strong className="text-xs text-blue-500">₹{price(event)}+</strong><ArrowRight size={15} className="muted"/></Link>)}</div></section>;
}
