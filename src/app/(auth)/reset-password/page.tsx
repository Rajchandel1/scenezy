...
function ResetPasswordContent(){
  ...
  useEffect(()=>{let active=true;(async()=>{try{if(params.get('error'))throw new Error('This reset link is invalid or has expired.');const supabase=createSupabaseBrowserClient();const {data}=await supabase.auth.getSession();if(!data.session)throw new Error('This reset link is invalid or has expired. Please request a new one.');if(active)setReady(true);}catch(err){if(active)setError(err instanceof Error?err.message:'This reset link is invalid or has expired.');}})();return()=>{active=false};},[params]);
